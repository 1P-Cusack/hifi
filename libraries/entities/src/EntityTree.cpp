...
bool EntityTree::readFromAFrame(const QByteArray &fileData) {
    bool success = true;
    typedef QMap<const EntityItemProperties *, EntityItemPointer> PropertyEntityMap;

    PropertyEntityMap propertyEntities;
    AFrameReader reader;
    success = reader.read(fileData);
    if (success) {

        // Attempt to create an entity for each property constructed by the reader.
        const AFrameReader::AFramePropList &propData = reader.getPropData();
        const AFrameReader::AFramePropChildMap &propChildMap = reader.getPropChildMap();
        foreach(const EntityItemProperties &properties, propData) {
            ...
            // Note the entity generated by the property instance
            propertyEntities.insert(&properties, entity);
        }

        // Attempt to link each created entity with its respective children, if any.
        AFrameReader::AFramePropChildMap::const_iterator const iterEndParentProp = propChildMap.cend();
        PropertyEntityMap::const_iterator const iterEndEntities = propertyEntities.cend();
        for (auto iterParentProp = propChildMap.cbegin(); iterParentProp != iterEndParentProp; ++iterParentProp) {
            auto iterParentEntity = propertyEntities.constFind(iterParentProp.key());
            if (iterParentEntity == iterEndEntities) {
                
                qCWarning(entities) << "Failed to find ParentEntity for EntityItemProperties:" << iterParentProp.key();

                // Early Iteration Exit
                continue;
            }

            const EntityItemID parentId = iterParentEntity.value()->getID();
            const AFrameReader::EntityItemPropertiesVec &childProps = iterParentProp.value();
            for each(const EntityItemProperties * const properties in childProps) {
                auto iterChildEntity = propertyEntities.constFind(properties);
                if (iterChildEntity == iterEndEntities) {
                    qCWarning(entities) << "Failed to find ChildEntity for EntityItemProperties:" << properties;

                    // Early Iteration Exit
                    continue;
                }

                iterChildEntity.value()->setParentID(parentId);
            }
        }
    } else {
